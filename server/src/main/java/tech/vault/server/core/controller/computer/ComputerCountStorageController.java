...
public class ComputerCountStorageController {
    @Autowired
    ComputerCountStorageService service;

    @GetMapping("/computer-storage")
    public ResponseEntity<Map<String, Map<Integer, Long>>> countComputersByLocation() {
        return ResponseEntity.status(HttpStatus.OK).body(service.countComputerStorage());
    }
}
